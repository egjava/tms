	...
	$(document).ready(function(e){	
		...
		function getTherapist(){
			$.ajax({
				...
				/*	$("#therapistName option").val(function(i,v){
						  $(this).siblings("[value='"+ v +"']").remove();
						});*/
			...
			$.ajax({
				...
				success: function (data, textStatus, jqXHR) {
					...
					$.each(data, function(index, aType) {
						...
						if(formid == 'client' || formid == 'payments' || formid == 'newactivity' || formid == 'searchnewactivity')	{
...
/*					
				...
				$('#newactivity').validate({
					...
					submitHandler: function(form) {
						...
						JSONObject= {	"nameid":			cnameid,
										...
										"paymentdate":		$('#actdate').val()
									};
						jsonData = JSON.stringify(JSONObject); 
						
						
						/*if(buttonvalue == 'Update')
							urlvalue = rooturl + "clientNewActivityUpdate" +"/" + doctorid
						*/	if(buttonvalue == 'Save')
		...
		function renderReportSummary(data,textStatus,error){
			...
			$('#therapistdetail').append(data[0].address1 + "," +data[0].address1+'<br>');
